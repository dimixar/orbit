...
- ✅ Workbench readers: skills (`skills.rs`), plugins (`plugins.rs`), usage history. ⬜ settings (on-disk) persistence.
...
- ✅ Extension dialogs (`select`/`confirm`/`input`/`editor`) render as a native modal (`dialog.rs`), covering `ask_user_question`'s multiple-choice and multi-select flows. ⬜ Richer per-tool renderers and approval dialogs (bash, edit, todo, plan, search, mcp, thinking).
...
- ✅ Git page (Changes / History / Graph + commit bar, one-shot commit-message generation). ✅ Settings → Providers: full built-in catalog, API-key + OAuth auth, models.json CRUD, refresh. ✅ Settings → Skills: SKILL.md discovery (project + global agent) with reveal. ✅ Settings → Plugins: pi package inventory + install/remove/update via the CLI. ⬜ Usage charts, settings persistence.
