...
| First run | `onboarding.rs` + `platform.rs` + `app/view.rs` | The setup page (`render_onboarding`) owns the main area while a required piece is missing, and is reachable on request from Settings → About → Requirements (the platform card there always shows the host). It probes `pi`, `node`, and `git` by name — `PATH`, Homebrew-style dirs, the home shim dirs (`.volta/bin`, `.bun/bin`, `mise`/`asdf` shims, nvm/fnm version dirs) — and on Windows tries `exe`/`cmd`/`bat` before the bare name, because npm ships an extensionless `pi` shell script beside the `pi.cmd` that `Command` can actually spawn. Each row carries a version from `<bin> --version` or the install command for *this* host (`winget install OpenJS.NodeJS.LTS` / `brew install node` / `apt`, npm for `pi`), and the pages below it report the host: `platform::host()` labels the OS (`Windows 11 (build 26100)`, `macOS 15.2`, `/etc/os-release`'s PRETTY_NAME) with its arch, and names the floor a bundle declares (macOS 13, Windows 10 1809 / build 17763). Under the dependencies sit the paths that decide whether the sidebar has anything in it — pi's session store and `~/.orbit-pi` — resolved through `platform::home_dir()` (`HOME` → `USERPROFILE` → `HOMEDRIVE`+`HOMEPATH`), so an Explorer or PowerShell launch (no `HOME`) still finds them. Refresh re-probes off the main thread; **Done** appears only when the page was opened on request |
