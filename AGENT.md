...
crates/orbit-pi/        GPUI app — window, shell, chat, settings
  ...
  src/app/updater_ui.rs signed-updater rows, pill, and event drain
  ...
  src/notifications.rs  background desktop banners (UNUserNotificationCenter; its delegate routes a clicked banner back to the session; osascript fallback when unbundled) + system alert sound; prefs in ~/.orbit-pi/notifications.json
  ...
  src/quota_bridge.rs   materializes the bundled `contrib/orbit-quota-extension/` under `~/.orbit-pi/` and spawns pi with `--extension`
...
| Workbench | `app/settings.rs` + `usage/` + `skills_ui.rs` | Settings sections General / Runtime / Agent / Skills / Plugins / **Models** / Appearance / Providers / About. **Models** lists the `get_available_models` catalog grouped by provider with search, context-window, active check, and a favorite star shared with the composer picker (`favorites.rs`). Usage / Skills / Plugins / Providers are full pages (see the Settings row). Clone session is offered from ⌘P. |
| Stubs | — | No rewind-to-turn UI (clone exists; `fork` needs entry ids). No native macOS menu bar. No per-tool permission dialogs (blocked on protocol). |
...
**Shipped infra:** GitHub Actions CI (`.github/workflows/ci.yml`, macOS build + test + clippy); `make-dmg.sh` signs, optionally notarizes and staples each `.app`/DMG when `NOTARY_PROFILE` is set; `transcript.rs` carries a 10k-message model-layer perf test.
...
6. ✅ Extension dialogs (`extension_ui_request`: `select`/`confirm`/`input`/`editor`) render natively and block the run until answered. ⬜ Residual: trigger a tool permission/approval in RPC mode and record whether it surfaces as an event (or confirm the `--approve` fallback).
...
- ✅ Sessions: list, open, switch, new, **clone** (⌘P), delete, hidden workspaces, cross-workspace. ⬜ rewind (`fork`) needs entry-id plumbing; rename is active-session-only.
...
- ✅ Extension dialogs (`select`/`confirm`/`input`/`editor`) as a native modal. ⬜ richer per-tool renderers (bash/thinking dedicated; edit/todo/plan/search/mcp are generic) and approval dialogs (blocked on protocol).
...
- ✅ Git page (Changes / History / Graph + commit bar, commit-message generation). ✅ Providers: built-in catalog, API-key + OAuth, models.json CRUD, refresh. ✅ Skills, ✅ Plugins (+ search), ✅ **Models** page, ✅ Usage charts + filters + export. ✅ Theme/UI persistence (Orbit store).

### P5 — Mermaid + accessibility (partial)
- ✅ Mermaid fences stay copyable code blocks with an honest label (never faked). ✅ Reduce-motion setting honored by shimmer/spinners/drop fade. ⬜ per-tool permission UI (blocked on protocol); ⬜ focus rings / screen-reader labeling.
...
- `cargo test --workspace` — unit tests + live pi integration tests. `live_pi.rs`, `live_catalog.rs`, and the other live tests skip when `pi` is missing; `orbit-pi` session-store tests skip when `~/.pi/agent/sessions` is empty. CI runs build + test + clippy on macOS (`.github/workflows/ci.yml`).
