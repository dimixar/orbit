...
crates/orbit-pi/        GPUI app — window, shell, chat, settings
  ...
  src/quota.rs          account quota/balance/spend reducer over the `quota.*` RPC (no secrets, no I/O)
...
crates/orbit-rpc/       pi CLI process + JSONL protocol
  ...
  tests/quota_fake_pi.rs scripted-server coverage of the quota.list round trip
