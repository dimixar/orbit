...
test("summarize prefers the salient argument and stays single-line", () => {
  assert.equal(summarize("bash", { command: "rm -rf build" }), "bash: rm -rf build");
  assert.equal(summarize("edit", { path: "src/a.rs" }), "edit: src/a.rs");
  const long = summarize("bash", { command: "x".repeat(500) });
  assert.ok(long.length <= 210, "truncated");
...
test("modeFilePath points at ~/.orbit-pi/access.json", () => {
