...
fn write_override(skill: &Skill, workspace: &Path, enabled: bool) -> Result<(), String> {
    ...
    serialized.push('\n');
    fs::write(&path, serialized).map_err(|err| tr!("errors.could_not_write", path = path.display().to_string(), error = err))
}
...
pub(crate) fn delete(skill: &Skill, workspace: &Path) -> Result<(), String> {
    ...
    if dir.file_name().and_then(|name| name.to_str()) == Some("skills") {
        fs::remove_file(&skill.file)
            .map_err(|err| tr!("errors.could_not_delete", path = skill.file.display().to_string(), error = err))?;
    } else {
        fs::remove_dir_all(dir)
            .map_err(|err| tr!("errors.could_not_delete", path = dir.display().to_string(), error = err))?;
    }
...
pub(crate) fn read_body(file: &Path) -> Result<String, String> {
    let raw = fs::read_to_string(file)
        .map_err(|err| tr!("errors.could_not_read", path = file.display().to_string(), error = err))?;
