//! Paint-only syntax highlighting for diffs and code.
//!
...
fn spec(lang: Lang) -> LangSpec {
    match lang {
        ...
        // Diff hunks are line-classified by `diff_line`; this spec is never
        // consulted.
        Lang::Diff => LangSpec {
            line_comments: &[],
            block_comment: None,
...
pub fn tokenize_line(lang: Lang, line: &str, carry: Carry) -> (Vec<Token>, Carry) {
    ...
    while index < bytes.len() {
        ...
        match spec.block_comment {
            Some((open, close)) if rest.starts_with(open) => {
...
///
/// The diff renderer tokenizes line by line because it interleaves additions
/// and deletions; this whole-block entry point keeps the lexer usable for a
/// future markdown/code-block caller (and is exercised by the tests).
#[allow(dead_code)]
